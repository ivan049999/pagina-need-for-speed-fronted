import type { GamePageContent } from "@/content/games/game-page-content";

const CARBON_ASSETS =
  "/images/juegos-need-for-speed/Need-For-Speed-Carbon";
const UNDERCOVER_ASSETS =
  "/images/juegos-need-for-speed/Need-For-Speed-Undercover";
const WORLD_ASSETS = "/images/juegos-need-for-speed/Need-For-Speed-World";

export const NEED_FOR_SPEED_WORLD: GamePageContent = {
  slug: "need-for-speed-world",
  title: "Need for Speed™ World",
  tags: ["Juego básico", "Carreras"],
  priceLabel: "Desde",
  price: "9,99 €",
  priceNote: "Pueden aplicarse impuestos aplicables en la compra.",
  ctaLabel: "Seleccionar Plataforma",
  storePlatforms: [
    {
      id: "ea",
      label: "Comprar en CeX España",
      href: "https://es.webuy.com/search?stext=need+for+speed+world",
      iconSrc: `${CARBON_ASSETS}/CeX_Logo.webp`,
    },
    {
      id: "xbox",
      label: "Comprar en Amazon España",
      href: "https://www.amazon.es/s?k=need+for+speed+world",
      iconSrc: `${CARBON_ASSETS}/logoAmazon.png`,
    },
    {
      id: "steam",
      label: "Comprar en Ebay",
      href: "https://www.ebay.com/sch/i.html?_nkw=need+for+speed+world",
      iconSrc: `${CARBON_ASSETS}/logoEbay.png`,
    },
  ],
  legalNote:
    "Need for Speed es una marca registrada de Electronic Arts Inc. Las marcas de terceros son propiedad de sus respectivos titulares.",
  rating: {
    system: "pegi",
    age: 3,
    ageBadgeSrc: `${CARBON_ASSETS}/Pegi-3-Icono.png`,
    descriptors: [
      {
        id: "violence",
        label: "Violence",
        iconSrc: `${CARBON_ASSETS}/violence-Icono.png`,
      },
    ],
  },
  heroVideoSrc:
    "/videos/Juegos-Need-For-Speed/Need-For-Speed-World/need-for-speed-world-video-1.mp4",
  coverImage: {
    id: "cover",
    src: `${WORLD_ASSETS}/need-for-speed-world-portada-oficial.jpg`,
    alt: "Carátula de Need for Speed World",
  },
  media: [
    {
      id: "hero-1",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-1.jpg`,
      alt: "Portada de Need for Speed World",
    },
    {
      id: "hero-2",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-2.avif`,
      alt: "Need for Speed World — captura 1",
    },
    {
      id: "hero-3",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-3.jpg`,
      alt: "Need for Speed World — captura 2",
    },
    {
      id: "hero-4",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-4.jpg`,
      alt: "Need for Speed World — captura 3",
    },
    {
      id: "hero-5",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-5.webp`,
      alt: "Need for Speed World — captura 4",
    },
    {
      id: "hero-6",
      src: `${WORLD_ASSETS}/need-for-speed-world-imagen-6.jpg`,
      alt: "Need for Speed World — captura 5",
    },
  ],
  about: {
    title: "Acerca del juego",
    description:
      "Need for Speed™ World fue el primer MMO gratuito de la saga: un mundo persistente en línea donde personalizabas tu coche, competías en carreras públicas y privadas, subías de nivel y desbloqueabas mejoras mientras explorabas un mapa abierto inspirado en ciudades reales.",
    platforms: "PC (Windows)",
    languages:
      "Checo, Alemán, Inglés, Español, Francés, Italiano, Holandés, Sueco, Danés, Noruego, Finlandés, Polaco, Portugués, Ruso",
    publisher: "Electronic Arts",
    releaseDate: "20 jul 2010",
  },
  systemRequirements: {
    title: "Requisitos del sistema",
    osLabel: "Windows",
    minimum: [
      { label: "Sistema operativo", value: "Windows XP SP2 / Vista" },
      {
        label: "Procesador",
        value: "Pentium 4 a 3,2 GHz o Athlon 64 3500+",
      },
      { label: "Memoria", value: "1 GB RAM (2 GB en Vista)" },
      { label: "Disco duro", value: "10 GB de espacio libre" },
      {
        label: "Tarjeta gráfica",
        value:
          "256 MB, compatible con DirectX 9.0c (GeForce 7600 / Radeon X1600)",
      },
      { label: "Tarjeta de sonido", value: "Compatible con DirectX 9.0c" },
    ],
    recommended: [
      { label: "Sistema operativo", value: "Windows Vista / 7" },
      { label: "Procesador", value: "Core 2 Duo a 2,0 GHz o equivalente" },
      { label: "Memoria", value: "2 GB RAM" },
      { label: "Disco duro", value: "10 GB de espacio libre" },
      {
        label: "Tarjeta gráfica",
        value: "512 MB o superior, compatible con DirectX 9.0c",
      },
      { label: "Tarjeta de sonido", value: "Compatible con DirectX 9.0c" },
    ],
  },
};
